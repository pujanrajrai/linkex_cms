...
def build_sgus_xml(details, username, password, security_key, auth_code, account_number, service_code):
    ...
    ET.SubElement(details_tag, "ServiceType").text = awb_info.get(
        "service_code").lower()
    ET.SubElement(details_tag, "ServiceCode").text = service_code.lower()
    ...
    # shipper details with default values
    shipper = ET.SubElement(shipment, "Shipper")
    ET.SubElement(shipper, "CompanyName").text = "SHIPGLOBAL"
    ET.SubElement(shipper, "ContactName").text = "SHIPGLOBAL"

    ET.SubElement(shipper, "Address1").text = "36 St Johns Place"
    ET.SubElement(shipper, "Address2").text = " "
    ET.SubElement(shipper, "City").text = "FREEPORT"
    ET.SubElement(shipper, "StateProvince").text = "NY"
    ET.SubElement(shipper, "ZipPostal").text = "11520"
    ET.SubElement(shipper, "Country").text = "US"
    ET.SubElement(shipper, "Phone").text = "212 382 1741"
    ET.SubElement(shipper, "Email").text = "shipglobalops@gmail.com"
    ET.SubElement(shipper, "Reference").text = ""
    ...
    ET.SubElement(receiver, "StateProvince").text = consignee.get(
        "state_short_name", "")
    ...
    for idx, box in enumerate(boxes, 1):
        ...
        # actual weight in kg
        actual_weight = box.get("actual_weight", 0)
        # actual weight in lbs
        actual_weight_lbs = float(actual_weight) * 2.20462
        actual_weight_lbs = math.ceil(actual_weight_lbs)
        # weight converting completed
        ET.SubElement(pkg, "Weight").text = str(actual_weight_lbs)
        ET.SubElement(pkg, "WeightType").text = "LB"

        dims = box.get("dimensions", {})
        length = dims.get("length")
        breadth = dims.get("breadth")
        height = dims.get("height")
        length_in_inches = float(length) * 0.393701
        length_in_inches = math.ceil(length_in_inches)
        breadth_in_inches = float(breadth) * 0.393701
        breadth_in_inches = math.ceil(breadth_in_inches)
        height_in_inches = float(height) * 0.393701
        height_in_inches = math.ceil(height_in_inches)
        ET.SubElement(pkg, "Length").text = str(length_in_inches)
        ET.SubElement(pkg, "Width").text = str(breadth_in_inches)
        ET.SubElement(pkg, "Height").text = str(height_in_inches)
        ET.SubElement(pkg, "DimUnit").text = "IN"
...
def sgus_api(awbno):
    ...
    if awb_obj.is_api_called_success:
        return {"success": True, "awbno": awbno, "message": "Already submitted to SGUS"}
    if not awb_obj.is_editable:
        return {"success": False, "awbno": awbno, "message": "AWB is not editable"}
    if awb_obj.vendor.code != "SGUS":
        return {"success": False, "awbno": awbno, "message": "AWB is not for SGUS"}
    if awb_obj.total_box > 1:
        return {"success": False, "awbno": awbno, "message": "SGUS does not support multiple boxes"}
    ...
    # --- Send XML to API ---
    try:
        url = "https://www.shipglobal.us/api/shipmentprocess"
        ...
        if result_tag.lower() == "success":
            ...
            return {
                "success": True,
                "awbno": awbno,
                "message": "Shipment submitted successfully to SGUS",
                "data": {
            ...
            failure_message = shipment.findtext(
                "Message", default="SGUS API responded with failure").strip()
            return {
                "success": False,
                "awbno": awbno,
        ...
        return {
            "success": False,
            "awbno": awbno,
            "message": f"Request to SGUS failed: {e}"
        }
