...
def build_sgca_xml(details, username, password, security_key, auth_code, account_number, service_code):
    ...
    for idx, box in enumerate(boxes, 1):
        ...
        ET.SubElement(pkg, "Currency").text = "CAD"
        ET.SubElement(pkg, "Content").text = awb_info.get("content", "")

    # Other fields
    ET.SubElement(shipment, "LabelType").text = "A4"
...
def sgca_api(awbno):
    ...
    if awb_obj.is_api_called_success:
        return {"success": True, "awbno": awbno, "message": "Already submitted to SGCA"}
    if not awb_obj.is_editable:
        return {"success": False, "awbno": awbno, "message": "AWB is not editable"}
    if awb_obj.vendor.code != "SGCA":
        return {"success": False, "awbno": awbno, "message": "AWB is not for SGCA"}
    if awb_obj.total_box > 1:
        return {"success": False, "awbno": awbno, "message": "SGCA does not support multiple boxes"}
    ...
    try:
        ...
        url = "https://www.firstflightcanada.com/api/shipmentprocess"
        ...
        if result_tag.lower() == "success":
            ...
            return {
                "success": True,
                "awbno": awbno,
                "message": "Shipment submitted successfully to SGCA",
                "data": {
            ...
            failure_message = shipment.findtext(
                "Message", default="SGCA API responded with failure").strip()
            return {
                "success": False,
                "awbno": awbno,
        ...
        return {
            "success": False,
            "awbno": awbno,
            "message": f"Request to SGCA failed: {e}"
        }
